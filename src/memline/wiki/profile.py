...
def profile_batches(
    ...
    *,
    ...
    def run(batch: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            data, result = call_json(
                prompt, job="profile", max_tokens=max_tokens,
                progress=lambda line, b=batch["batch_id"]: log(f"{b}: {line}"))
...
def profile_sources(
    ...
    *,
    ...
    def run(path: Path) -> dict[str, Any]:
        ...
        try:
            data, result = call_json(
                render(template, batch, material), job="profile",
                max_tokens=max_tokens,
                progress=lambda line, r=rel: log(f"{r}: {line}"))
        except RefusedError as exc:
