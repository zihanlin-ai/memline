...
session_id=ses_b8d2ac181351976b11df6be5bb
writer_agent_id=opencode
...
Historical ledger entries share one explicitly synthetic OpenCode scope:
`agent_id=opencode` and
`run_id=session_id=ses_b8d2ac181351976b11df6be5bb`. The synthetic session is
only writer/session identity. `source=agent-memory-ledger`,
`origin=ledger_import`, `import_batch=ledger-YYYY-MM`, and the ledger
date/file/line fields retain the actual import provenance and event ordering.
Do not replace those provenance fields with the synthetic session.
...
manifests are `.agent-memory/manifests/metadata-backfill-*.jsonl` and
`.agent-memory/manifests/ledger-identity-migration-*.jsonl`.

## Timestamp Policy

`memline add` automatically records current `created_at`, `ledger_timestamp`, and `ingested_at`.
...
```bash
memline-ingest-ledger path/to/ledger.md --month 2026-05 --mode line --dry-run
```
