# Staleness & Supersession Design for memline

Status: steps 1-5 complete. Steps 1-4 MERGED to main of
`.agent-memory/projects/memline` (merge 0d5e8a1) and EXPOSED in the
...
6. **Keep the stack** — memline + Qdrant + manifests + event queue. No
...
- **Decided 2026-07-17:** the judge reuses the existing `[llm]` endpoints
  rather than owning a model choice. The async queue absorbs their latency
  (~160 calls/day is well within background throughput); per-call cost is
  accepted.
- `[llm]` is an ordered endpoint list: the primary, then any `[llm.fallback]`
  entries. Each call tries them in order and falls over on transport or
  provider failure, so a judge never stalls on one vendor being unreachable,
  rate-limited, or off-network. Each endpoint carries its own credential and
  its own `extra_body` patch (provider pinning, routing hints); a fallback
  may not inherit the primary's credential. Pairs are stamped with whichever
  endpoint actually answered, not the configured preference — so
  `judge_model` in the pair store is evidence, not intent.
- Results are written as suspicion-pair records. No state change ever happens
  here.

### 3.3 Handoff review (the disposition point)

`memline review --session [<id>]` (session auto-detected like add/search):
...
`event`/`status`) show "N open staleness suspicions — run `memline stale
...
6. **Superseder itself deleted/invalidated** — never auto-revive its victims.
   ...
   `memline revive <id>` clears `superseded_by` (audited).
...
memline package; bump the version suffix to `+workspace.2`. Future
