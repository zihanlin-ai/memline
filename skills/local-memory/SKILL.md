...
- At session start or before multiple memory operations, run `mem0-local daemon status`. If it is not running and memory commands are expected to be repeated, start it with `mem0-local daemon start`; the daemon is optional, local, and not expected to auto-start at boot. If it is unavailable, continue with the direct CLI path.
