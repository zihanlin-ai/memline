...
- The vector store backend is selected by `config.toml`: when `[vector_store]` host/port is set (this workspace's setup), memory commands talk to a local qdrant server, and that server process must be running. If memory commands fail with a connection error (typically after a machine/WSL restart), start it first with `.agent-memory/store/qdrant-server/qdrantctl.sh start` (check with `... status`). Without `[vector_store]`, the embedded local-path store is used and no extra process is needed.
...
- Write small, frequent, single-fact entries — not big multi-fact paragraphs. One `add` per atomic fact, called often, beats one long dense entry. This matches Mem0's one-fact-per-memory retrieval model and avoids the LLM-extraction backend truncating long entries. With `mem0ai>=2.0.11`, an LLM-extraction failure surfaces as a CLI error (exit code 1) instead of silently returning empty `results`: on error, retry or split the input into shorter atomic entries; on empty `results` with success status, the entry was deduplicated against an existing memory or contained no extractable new fact — usually no action needed, but rewrite as a more concrete single fact if the content genuinely should have been stored.
