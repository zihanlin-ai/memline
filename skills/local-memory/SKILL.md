...
- Auto-start is namespace-guarded: from an isolated sandbox context the CLI never spawns a duplicate daemon or qdrant. When a fresh host daemon exists, ordinary commands transparently fall back from the inaccessible Unix socket to the shared SQLite mailbox and keep the exact same CLI, handlers, store, audit, and writer/session attribution. Host, Claude, and OpenCode callers keep using the Unix socket. If no host daemon is alive, start it once from a normal host-visible shell; `MEMLINE_NO_DAEMON=1` remains a debugging-only direct-path override.
